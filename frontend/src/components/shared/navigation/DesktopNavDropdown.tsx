...
const DesktopNavDropdown: React.FC<DesktopNavDropdownProps> = ({
  ...
  const handleInternalClick = (index: number, href: string): void => {
  ...
  const handleExternalClick = (index: number): void => {
    setSelectedImage(index);
    if (onItemSelected) {
      onItemSelected();
    }
          ...
          {item.children.map((child, i) => {
            ...
            return (
              <div
                key={child.href}
                className="group relative overflow-hidden transition-all duration-300 hover:scale-105"
                onClick={() => handleInternalClick(i, child.href)}
              >
                {tileContent}
              </div>
            );
