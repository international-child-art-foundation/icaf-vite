...
import {
    UpdateSeasonSubmissionLimitRequest,
    UpdateSeasonSubmissionLimitResponse,
    validateUpdateSeasonSubmissionLimitRequest
...
export const handler = async (event: ApiGatewayEvent) => {
    try {
        ...
        let requestData: UpdateSeasonSubmissionLimitRequest;
        try {
        ...
        const validationErrors = validateUpdateSeasonSubmissionLimitRequest(requestData);
        ...
        const oldMaxSubmissions = season.max_user_submissions || 1;

        // 5) Check if value is the same
        if (oldMaxSubmissions === requestData.max_user_submissions) {
            return CommonErrors.badRequest(`Submission limit is already set to ${requestData.max_user_submissions}`);
        }

        // 6) Update season submission limit
        ...
        await dynamodb.send(new UpdateCommand({
            ...
            UpdateExpression: 'SET max_user_submissions = :max_user_submissions, updated_at = :updated_at',
            ExpressionAttributeValues: {
                ':max_user_submissions': requestData.max_user_submissions,
        ...
        await dynamodb.send(new PutCommand({
            ...
            Item: {
                ...
                action_type: 'update_season_submission_limit',
                admin_user_id: adminUserId,
                season: seasonName,
                reason: requestData.reason.trim(),
                old_value: oldMaxSubmissions,
                new_value: requestData.max_user_submissions,
                timestamp: timestamp,
        ...
        const response: UpdateSeasonSubmissionLimitResponse = {
            message: 'Season submission limit updated successfully',
            season: seasonName,
            season_name: colloqName,
            old_max_user_submissions: oldMaxSubmissions,
            max_user_submissions: requestData.max_user_submissions,
            is_active: isActive,
            admin_action_id: adminActionId,
            timestamp: timestamp
        ...
        console.error('Error in updateSeasonSubmissionLimit handler:', error);
        ...
        return CommonErrors.internalServerError('Failed to update season submission limit');
    }
};
